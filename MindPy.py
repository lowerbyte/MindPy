    ...
    while True:
        ...
        if ch == ord(':'):
            ...
            if user_input[0] == 's' and not root:
                ...
                # print instruction
                hmi.addstr('Set cursor position using arrow keys.'
                           ' Hit p to put text.')
                hmi.refresh()
                cpos = win.pad.getch()
                while(cpos != ord('p')):
        ...
        # keys responsible for moving the pad
            ...
            hmi.addstr('Incorrect command. Type :h for '
...
if __name__ == '__main__':
    ...
    parser.add_argument('--draw', help='draw [option] '
                        '- select drawing option (diagonal)')
    parser.add_argument('--load', help='load [file] '
                        '- load serialized json file')
